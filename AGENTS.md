...
1. Изменить страницы, компоненты, стили или файлы в `public/`. Не размещать CRM-ассеты в основном репозитории сайта или в `market`.
2. Локально проверить изменения:
   ...
   Production build должен завершиться без ошибок и создать `dist/` с ссылками, учитывающими base path `/crm`.

3. Для локальной публикации в авторизованном окружении выполнить `npm run deploy:prod`. Скрипт загружает только объекты в `s3://xn--g1acsdbq.xn--p1ai/crm/` и не должен получать другой `DEPLOY_PREFIX`.
4. Обычная публикация выполняется автоматически после push в `main`; также workflow **Deploy CRM** можно запустить вручную в GitHub Actions. Он использует `YC_SA_JSON_CREDENTIALS` и ограниченный IAM-ролью `storage.uploader` аккаунт `zipstop-sa`.
5. После завершения workflow открыть `https://зиптоп.рф/crm` и проверить нужные страницы. HTML не кешируется надолго, но ассеты в `_astro/` могут иметь immutable cache: после изменения они должны получать новые хешированные имена.

Не удалять объекты за пределами `crm/`. Если нужно удалить устаревший файл внутри этого префикса, сначала подтвердить точный object key и выполнить отдельную безопасную операцию: текущий CI загружает новые или изменённые файлы, но не очищает bucket.
