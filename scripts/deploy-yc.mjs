...
const run = promisify(execFile), bucket = process.env.BUCKET_NAME || 'xn--g1acsdbq.xn--p1ai', prefix = (process.env.DEPLOY_PREFIX || 'crm').replace(/^\/+|\/+$/g, ''), dist = join(process.cwd(), 'dist'), yc = process.env.YC_PATH || 'yc';
...
for (const file of await files(dist)) { const path = relative(dist, file), args = ['storage','s3api','put-object','--bucket',bucket,'--key',`${prefix}/${path}`,'--body',file,'--content-type',types[extname(file)] || 'application/octet-stream']; if (path.startsWith('_astro/')) args.push('--cache-control','public, max-age=31536000, immutable'); if (path.endsWith('.html')) args.push('--cache-control','public, max-age=0, must-revalidate'); await run(yc, args); console.log(`Uploaded ${prefix}/${path}`); }
console.log('Deployment complete: https://зиптоп.рф/crm');
